...
set<string> ignore_reagents;

const short ind_num = 3;
const char ind_char = ' ';
bool doScale = true;

size_t getScale(const reagent& reag, const set<string>& traversed_in) {
	...
	set<string> traversed(traversed_in);
...
size_t getScale(const reagent& reag) {
	return getScale(reag, set<string>());
}
void analyzeReagent(const reagent& reag, unordered_map<string, double>* amounts_in, short indent_n, double req_out, const set<string>& traversed_in) {
	set<string> traversed(traversed_in);
	traversed.insert(reag.name);
...
void analyzeReagent(const reagent& reag, short indent_n, double req_out) {
	analyzeReagent(reag, nullptr, indent_n, req_out, set<string>());
}
